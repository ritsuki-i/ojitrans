...
def jaoji(oji):
    ...
    if start=="やすみ":
        ...
        start="お早う"
    try:
        sentence = loaded_model.make_sentence_with_start(beginning=start, strict=False).replace(' ', '')
    except markovify.text.ParamError:
        sentence = loaded_model.make_short_sentence(100,20,tries=100).replace(' ', '')
    else:
        sentence = loaded_model.make_sentence_with_start(beginning=start, strict=False).replace(' ', '')
    input = result[:-1]
    input="".join(input)
...
def ans():
    ...
    if Trans.mode == None or Trans.sentence == "":
        message = "選択してください"
        return render_template(
            'index.html', sentence=Trans.sentence, message=message
        )
    else:
        if Trans.mode == "おじさん構文→日本語":
            ...
            return render_template(
                'index.html', sentence=Trans.sentence, trans=Trans.trans, mode=Trans.mode, twitter=Trans.twitter
            )
        elif Trans.mode == "日本語→おじさん構文":
            Trans.trans = jaoji(Trans.sentence)
            Trans.twitter = "「{}」\n翻訳後→\n「{}」\n".format(Trans.sentence,Trans.trans)
            return render_template(
                'index.html', sentence=Trans.sentence, trans=Trans.trans, mode=Trans.mode, twitter=Trans.twitter
            )
...
def copy():
    ...
    if Trans.mode == None or Trans.sentence == "":
        ...
        return render_template(
            'index.html', sentence=Trans.sentence, trans=Trans.trans, mode=Trans.mode
        )
